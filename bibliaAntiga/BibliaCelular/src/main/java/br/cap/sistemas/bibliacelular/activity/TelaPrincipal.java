...
import br.cap.sistemas.bibliacelular.db.util.ConectionUtil;

import static br.cap.sistemas.bibliacelular.db.tabelas.CapitulosDao.gravaCapitulosDB;

import lib.kingja.switchbutton
...
public class TelaPrincipal extends BaseActivity  {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(ConectionUtil.netIsConnected(mContext)) {
            initDados();
        } else {
            // offline
            Toast.makeText(getApplicationContext(), "OffLine - Uso restrito", Toast.LENGTH_SHORT).show();
        }
    ...
    private void initDados() {
        ...
        if(!jsonlido) {  // só carrega na primeira vez.
            ...
            carrega_data_capitulo_dinamico(); // Carrega dados (capitulos) (Dinamico)
            SharedPreferencesUtil.save(this, this.getString(R.string.shared_json_lido), String.valueOf(true));
        }

        //PutAway putAway = new PutAway();
            //putAway.setBname("CPINTOACCENT");
            //putAway.setLgnum("B13");
            //putAway.setTanum("");
            //MyApplication.database.PutAwayDao().insert(putAway);

            // DialogUpload.showDialogUpload(this);

            //MyApplication.database.VanningConteinerDao().deleteAllRegisterVanningContainer();
            //MyApplication.database.SealConteinerDao().deleteAllRegisterSealContainer();
            //MyApplication.database.PutAwayDao().deleteAllRegisterPutAway();
            //MyApplication.database.ComponentPickingDao().deleteAllRegisterComponentPicking();
            //MyApplication.database.PackingTempDao().deleteAllRegisterPackingTemp();
            //MyApplication.database.PackingDao().deleteAllRegisterPacking();
            //MyApplication.database.InventoryDao().deleteAllRegisterInventory();
